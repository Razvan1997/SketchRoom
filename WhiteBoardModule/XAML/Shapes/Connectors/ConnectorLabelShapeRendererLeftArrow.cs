...
namespace WhiteBoardModule.XAML.Shapes.Connectors
{
    public class ConnectorLabelShapeRendererLeftArrow : IShapeRenderer, IStrokeChangable, IForegroundChangable, IRestoreFromShape
    {
        ...
        {
            if (_gridPanel?.Tag is not Dictionary<string, object> tag) return;
        ...
        {
            if (_gridPanel == null || control is not FrameworkElement fe)
                return null;
            ...
            if (_gridPanel.Tag is Dictionary<string, object> tag)
            {
                if (tag.TryGetValue("LabelText", out var lblObj) && lblObj is TextBox label)
                {
                    ...
                    labelFontSize = label.FontSize.ToString();
                    labelFontWheight = label.FontWeight.ToString();
                }
            ...
            return new BPMNShapeModelWithPosition
            {
                ...
                ExtraProperties = new Dictionary<string, string>
                {
                    ...
                    { "LabelFontWeight", labelFontWheight ?? "" }
                }
            };
        }
        ...
        public void Restore(Dictionary<string, string> extraProperties)
        {
            ...
            if (tag.TryGetValue("LabelText", out var lblObj) && lblObj is TextBox label)
            {
                ...
                if (extraProperties.TryGetValue("LabelFontWeight", out var fontWeightStr))
                {
                    ...
                    try
                    {
                        label.FontWeight = (FontWeight)fontWeightConverter.ConvertFromString(fontWeightStr);
                    }
                    catch
                    {
            ...
            if (extraProperties.TryGetValue("LineColor", out var lineColor))
            {
                ...
                if (tag.TryGetValue("LeftLine", out var leftObj) && leftObj is Shape left)
                    left.Fill = brush;

                if (tag.TryGetValue("RightLine", out var rightObj) && rightObj is Shape right)
                    right.Fill = brush;

                if (tag.TryGetValue("Arrow", out var arrowObj) && arrowObj is Shape arrow)
                    arrow.Fill = brush;
            }
        }
    }
